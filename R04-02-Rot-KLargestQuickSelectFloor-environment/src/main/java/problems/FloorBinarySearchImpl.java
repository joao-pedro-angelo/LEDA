...
public class FloorBinarySearchImpl implements Floor {
	...
	public Integer floor(Integer[] array, Integer x) {
		...
		if (array != null && array.length > 0 && x != null) {
			...
			result = floorRecursivo(array, x, null, 0, array.length - 1);
		}
	...
	private Integer floorRecursivo(Integer[] array, Integer x, Integer floor, int leftIndex, int rightIndex) {
		...
		if (leftIndex <= rightIndex) {
			...
			if (array[meio].compareTo(x) > 0) {
				result = floorRecursivo(array, x, floor, leftIndex, meio - 1);
			}

			if (array[meio].compareTo(x) < 0) {
				result = floorRecursivo(array, x, array[meio], meio + 1, rightIndex);
			}
