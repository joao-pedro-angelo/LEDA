...
public class QuickSelect<T extends Comparable<T>> {
	...
	public T quickSelect(T[] array, int k) {
		if (array != null && array.length > 0) {
			return array[quickSelectRecursive(array, k, 0, array.length - 1)];
		} return null;
	}
	
	private int quickSelectRecursive(T[] array, int k, int left, int right) {
		int result = -1;
		if (left <= right) {
			int middle = (left + right) / 2;
			if (middle < right && array[middle + 1].compareTo(array[middle]) > 0) {
				return quickSelectRecursive(array, k, middle + 1, right);
			}
			else if (middle > 0 && array[middle - 1].compareTo(array[middle]) < 0) {
				return quickSelectRecursive(array, k, left, middle - 1);
			}
			else {
				return middle;
			}
