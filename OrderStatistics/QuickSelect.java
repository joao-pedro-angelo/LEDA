...
public class QuickSelect<T extends Comparable<T>> {
	...
	public T quickSelectRecursivo(T[] array, int k, int left, int right){
		int indexPivot = particiona(array, left, right);
		if (k - 1 < indexPivot) {
			return quickSelectRecursivo(array, k, left, indexPivot - 1);
		}
		else if (k - 1 > indexPivot) {
			return quickSelectRecursivo(array, k, indexPivot + 1, right);
		}
		else return array[indexPivot];
	}
	
	public int particiona(T[] array, int left, int right) {
		int i = left;
		T pivot = array[i];
